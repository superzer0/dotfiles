...
# under ~/repo. Changes belong in a worktree under ~/repo/claude-worktrees.
...
case "$dir/" in
  ...
  "$ROOT"/*) ;;              # under ~/repo - keep checking
  *) exit 0 ;;               # outside ~/repo - not this policy's business
esac
...
[ -n "$top" ] || exit 0  # loose file under ~/repo, not in any repo
...
jq -n --arg repo "$repo" --arg top "$top" '{
  hookSpecificOutput: {
    ...
    permissionDecisionReason: (
      "Worktree-only policy for ~/repo: no direct changes in \($top).\n" +
